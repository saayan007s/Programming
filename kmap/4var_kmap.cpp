...
void deb()
{
    ...
    cout << "debugging prefix sum array" << endl;
    // fur(i, 0, 3) {
        // fur(j, 0, 3)
            // cout << s[i][j] << ' ';
        // cout << endl;
    ...
    cout << "debugging dims" << endl;
    // for(auto u : dims)
        // cout << u.area << ' ' << u.l << ' ' << u.w << endl;
    // cout << endl;

    cout << "debugging final groups" << endl;
...
string exp_sop(int l, int r, bool row)
{
    if(l >= 4 && r >= 4) {
        l -= 4;
        r -= 4;
...
int main() 
{
    ...
    cout << "If you would like to calculate" << endl << "- SOP : Enter 0" <<  endl << "- POS : Enter 1" << endl;
    cin >> pos;

    cout << "Enter the numbers in the cardinal format separated by a space. When done enter \"-1\"" << endl;
    vi a;
    while(true){
        int temp;
        cin >> temp;
    ...
    if(!pos) {
        string ans = "";
        for(auto u : final_grps) {
            string tmp1 = exp_sop(u.x, u.X, 1);
            string tmp2 = exp_sop(u.y, u.Y, 0);

            if(tmp1 != "true" && tmp2 != "true")
                ans = ans + (ans.length() != 0 ? " + " : "") + tmp1 + tmp2;
            else if(tmp1 != "true")
                ans = ans + (ans.length() != 0 ? " + " : "") + tmp1;
            else if(tmp2 != "true")
                ans = ans + (ans.length() != 0 ? " + " : "") + tmp2;
        }
        cout << (ans.length() != 0 ? ans : "1") << endl;
    }
    else {
        string ans = "";
        for(auto u : final_grps) {
            ...
            cout << "(" << u.x << ", " << u.y << "); (" << u.X << ", " << u.Y << ")" << " : " << tmp1 << " : " <<  tmp2 << endl;
